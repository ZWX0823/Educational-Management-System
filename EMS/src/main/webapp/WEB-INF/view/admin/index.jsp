...
<html style="background-image: url('${pageContext.request.contextPath}/img/d.png')">
<head>
    <title>管理员主页</title>
...
<body onload="check()">
<%--接收消息--%>
<input type="hidden" id="delMsg" value="${deleteMessage}">
<!--顶栏-->
                    ...
                    <c:forEach items="${courseList}" var="item">
                        <tr>
                            ...
                            <td>
                                ...
                                <button class="btn btn-default btn-xs btn-success" onclick="location.href='${pageContext.request.contextPath}/admin/addTeacherCourse?courseID=${item.courseID}&deptID=${item.deptID}'">安排教师</button>
                            </td>
                        </tr>
                    </c:forEach>
                    </tbody>
                ...
                <div class="panel-footer">
                    <c:if test="${pagingVO != null}">
                        <nav style="text-align: center">
                            <ul class="pagination">
                                <li><a href="${pageContext.request.contextPath}/admin/index?page=${pagingVO.prePageNo}">&laquo;上一页</a></li>
                                <li class="active"><a href="">${pagingVO.currentPageNo}</a></li>
                                <c:if test="${pagingVO.currentPageNo+1 <= pagingVO.totalPageCount}">
                                    <li><a href="${pageContext.request.contextPath}/admin/index?page=${pagingVO.currentPageNo+1}">${pagingVO.currentPageNo+1}</a></li>
                                </c:if>
                                <c:if test="${pagingVO.currentPageNo+2 <= pagingVO.totalPageCount}">
                                    <li><a href="${pageContext.request.contextPath}/admin/index?page=${pagingVO.currentPageNo+2}">${pagingVO.currentPageNo+2}</a></li>
                                </c:if>
                                <c:if test="${pagingVO.currentPageNo+3 <= pagingVO.totalPageCount}">
                                    <li><a href="${pageContext.request.contextPath}/admin/index?page=${pagingVO.currentPageNo+3}">${pagingVO.currentPageNo+3}</a></li>
                                </c:if>
                                <c:if test="${pagingVO.currentPageNo+4 <= pagingVO.totalPageCount}">
                                    <li><a href="${pageContext.request.contextPath}/admin/index?page=${pagingVO.currentPageNo+4}">${pagingVO.currentPageNo+4}</a></li>
                                </c:if>
                                <li><a href="${pageContext.request.contextPath}/admin/index?page=${pagingVO.totalPageCount}">最后一页&raquo;</a></li>
                            </ul>
                        </nav>
                    </c:if>
                </div>
...
<script type="text/javascript">
    ...
    function deleteCourse(courseID){
        var msg = "你确认要删除该课程吗？";
        if (confirm(msg)==true){
            location.href="${pageContext.request.contextPath}/admin/deleteCourse?courseID="+courseID;
            return true;
        }
        else {
            return false;
